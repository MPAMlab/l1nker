...
try {
    ...
    if (typeof envSecretKey !== 'string' || envSecretKey.trim() === '') {
        throw new Error('JWT_SECRET_KEY environment variable must be a non-empty string.');
    }

      // 编码密钥
    secretKey = new TextEncoder().encode(envSecretKey);
  console.log('JWT_SECRET_KEY loaded successfully from environment.');
} catch (error) {
   console.error('Failed to load JWT_SECRET_KEY:', error.message);
   // 抛出错误，阻止 Worker 部署，并且输出错误信息
   throw new Error(`Failed to load JWT_SECRET_KEY: ${error.message}`);
}
