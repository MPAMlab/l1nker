   ...
   <div>
     ...
     <div v-if="data && data.length > 0">
       ...
       <el-dialog
         ...
         :title="`Edit ${selectedItem.redirectKey}`"
         width="80%"
       ...
       <el-dialog v-model="showCreateModal" title="Create New Item" width="80%" @close="closeCreateModal">
           <el-form :model="newItem" label-width="150px">
            ...
            <el-upload
             class="upload-demo"
              :action="'/api/upload'"
              :on-success="handleNewProfileImageUploadSuccess"
             :before-upload="beforeUpload"
             :show-file-list="false"
             >
              <el-button type="primary">
          ...
          <el-form-item label="Buttons">
             <el-button type="primary" @click="addButtonItem"><el-icon><Plus /></el-icon>Add Button</el-button>
            <div v-if="parsedNewButtons && parsedNewButtons.length > 0" style="margin-top: 10px;">
                <div v-for="(button, index) in parsedNewButtons" :key="index" style="border: 1px solid #eee; padding: 10px; margin-bottom: 10px; border-radius: 4px;">
                     <div style="margin-bottom: 10px;">
                         <span style="font-weight: bold;">Button #{{index + 1}}:</span>
                      </div>
                   <el-form-item label="Text">
                   ...
                   <el-form-item label="Is Download">
                     ...
                     <el-button type="danger" size="small" @click="removeButtonItem(index)"><el-icon><Delete /></el-icon>Delete</el-button>
                </div>
            </div>
             <div v-else>
               <el-empty description="No buttons available."  />
           ...
           <el-image
             style="width: 32px; height: 32px; margin-right: 10px;"
            :src="newItem.faviconUrl"
            fit="cover"
           />
           <el-upload
             class="upload-demo"
             :action="'/api/upload'"
              :on-success="handleNewFaviconUploadSuccess"
             :before-upload="beforeUpload"
            :show-file-list="false"
            >
              <el-button type="primary">
 ...
 export default {
   ...
   methods: {
       ...
       try {
         const response = await fetch(`/api/admin/data/${this.selectedItem.id}`, {
           ...
           headers: {
             ...
             body: JSON.stringify({...this.selectedItem, buttons: this.parsedButtons}),
         });
         if (!response.ok) {
           const errorData = await response.json();
         ...
         const response = await fetch('/api/admin/data', {
           ...
           headers: {
             ...
             Authorization: `Bearer ${localStorage.getItem('authToken')}`
              ...
              body: JSON.stringify({...this.newItem, buttons: this.parsedNewButtons}),
         });
         if (!response.ok) {
           const errorData = await response.json();
     ...
     async updateRedirectKey() {
       try {
         const response = await fetch(
           `/api/admin/data/update-redirect-key/${this.selectedItem.id}`,
           {
             method: 'PUT',
             headers: {
               'Content-Type': 'application/json',
     ...
     handleProfileImageUploadSuccess(response) {
      ...
      handleNewProfileImageUploadSuccess(response) {
        this.newItem.profileImageUrl = response.imageUrl;
     },
     handleFaviconUploadSuccess(response) {
       this.selectedItem.faviconUrl = response.imageUrl;
     },
     handleNewFaviconUploadSuccess(response) {
        this.newItem.faviconUrl = response.imageUrl;
    },
     beforeUpload(file) {
     ...
     removeButton(index) {
       ...
       addButtonItem() {
          this.parsedNewButtons.push({ text: '', link: '', isDownload: false });
       },
      removeButtonItem(index) {
           this.parsedNewButtons.splice(index, 1);
      },
